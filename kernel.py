...
========================================================================
Contains:
  1. mahal_fitness_kernel  - Mahalanobis J_M + Phi_PD + Phi_DTI per firefly (eq 4)
...
RTX A5000 / sm_86 Kernel Engineering (Section IV):
  ...
  | Constant-like access     | Sigma-  via const double* (L2 hit)  |
 ...
 *
 * Sigma-  is passed as a const double* pointer; the A5000's 6 MB L2 cache
 ...
 * __syncthreads - only 1 __syncthreads call per block total (vs 9 na ve).
 *
 * Parameters
 * ----------
 * X            : (n, d)          standardised feature matrix, row-major
 * centroids    : (N_ff, k, d)    all firefly centroid sets
 * sigma_inv    : (d, d)          precision matrix Sigma- 
 * pd_col       : (n,)            PD values; treated as zeros when all-zero
 * out_jm       : (N_ff,)         J_M per firefly  (eq 4)
 * out_phi_pd   : (N_ff,)         Phi_PD (within-cluster PD  sum)
 * out_phi_dti  : (N_ff,)         Phi_DTI (within-cluster DTI  sum)
...
__global__ void mahal_fitness_kernel(
    ...
    for (int row_start = 0; row_start < n; row_start += TILE) {
        ...
        for (int local_row = tid; local_row < rows_this; local_row += blockDim.x) {
            ...
            /* Phi_PD: within-cluster PD  (variance proxy)                     */
            const double pd = pd_col[global_row];
            acc_pd += pd * pd;

            /* Phi_DTI: standardised feature index 1 (DTI ratio)               */
